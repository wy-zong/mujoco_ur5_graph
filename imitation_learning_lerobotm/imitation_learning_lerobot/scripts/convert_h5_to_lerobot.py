...
def create_empty_dataset(env_cls: Type[Env]):
    ...
    print("[debug] dataset.features['task'] =", dataset.features.get("task"))

    return dataset


def populate_dataset(env_cls: Type[Env], dataset: LeRobotDataset):
    task = env_cls.name  # 你要存的任務名稱（字串最安全）
        ...
        with h5py.File(item, 'r') as root:
            ...
            for j in range(episode_length):
                frame = {
                    ...
                    **{
                        f'observation.images.{camera}': root[f'/observations/pixels/{camera}'][j]
                        for camera in env_cls.cameras
                    },
