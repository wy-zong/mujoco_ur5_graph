...
def main():
    ...
    # model_id = "google/siglip-so400m-patch14-384"  # SigLIP So400m (Patch size = 14, 384x384)
    ...
    for idx in frame_indices:
        ...
        # 將其轉換為 PIL Image，以符合 SigLIP processor 的輸入格式
    ...
    print(f"正在載入 SigLIP 模型: {model_id}...")
    processor = AutoImageProcessor.from_pretrained(model_id)
    model = AutoModel.from_pretrained(model_id)
    vision_model = model.vision_model  # 取出視覺編碼器
    vision_model.eval() # 設定為評估模式
    ...
    with torch.no_grad():
        for img in images:
            # SigLIP processor 會將影像 Resize 到 384x384
            inputs = processor(images=img, return_tensors="pt")
            
            # 只取 pixel_values 傳入 vision_model
            outputs = vision_model(pixel_values=inputs["pixel_values"])
            
            # 獲取最後一層的隱藏狀態
            last_hidden_states = outputs.last_hidden_state
            
            # SigLIP 沒有 CLS token，所有 token 都是 patch token
            # 對於 patch14 + 384x384: grid_size = 384 / 14 = 27, 共 729 個 patch
            patch_tokens = last_hidden_states[0]  # shape: (num_patches, hidden_dim)
    ...
    # 將高維度 (1152 維) 降至 3 維 (對應 R, G, B)
    pca = PCA(n_components=3)
    pca.fit(all_tokens_concat)

    # 建立畫布準備繪圖
    fig, axes = plt.subplots(len(images), 2, figsize=(10, 4 * len(images)))
    fig.suptitle("SigLIP-So400m Feature PCA Visualization", fontsize=16)
    ...
    for idx, (tokens, info) in enumerate(zip(all_patch_tokens, processed_images_info)):
        ...
        ax_pca.set_title("SigLIP-So400m PCA Features")
        ax_pca.axis("off")

    plt.tight_layout()
    plt.savefig("siglip_pca_visualization.png")
    print("完成！請查看產生的 siglip_pca_visualization.png 圖片。")
